...
class OrderController extends Controller
{
        ...
        public function bookingtable(Request $request)
    {
        try {
            ...
            $validatedData = $request->validate([
                ...
                'guest_numbers' => 'required|integer',
                'reservation_date' => 'required|date',
                'reservation_time' => 'required|date_format:H:i',
                'notes' => 'nullable|string',
                'where_to_seat' => 'required|in:inside,outside,bar,sidebar,NonSmoker,Other'
            ]);
            ...
            UserActivity::create([
                ...
                'activity_type' => 'appointment', // all, payments ,appointment, orders, rides, others
                'description' => 'Reservation for ' . $validatedData['guest_numbers'] . ' guests on ' . $validatedData['reservation_date'] . ' at ' . $validatedData['reservation_time'],
                'activity_date' => now()
            ]);
            ...
            return response()->json(['data' => ['message' => 'Reservation created successfully', 'reservation' => $reservation, 'user' => $user]], 201);
        } catch (\Exception $e) {
            // Handle exceptions
    ...
    public function proceed_place_order(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'order_amount' => 'nullable',
            'payment_method' => 'nullable|in:cash_on_delivery,digital_payment,wallet,offline_payment',
            'order_type' => 'nullable|in:take_away,delivery,parcel',
            'distance' => 'nullable',
            'address' => 'nullable',
            'longitude' =>  'nullable',
            'latitude' =>  'nullable',
            'parcel_category_id' =>  'nullable',
            'receiver_details' =>  'nullable',
            // 'charge_payer' => 'required_if:order_type,parcel|in:sender,receiver',
            'dm_tips' => 'nullable|numeric',
            'guest_id' => $request->user ? 'nullable' : 'nullable',
            'contact_person_name' => $request->user ? 'nullable' : 'nullable',
            'contact_person_number' => $request->user ? 'nullable' : 'nullable',
            'list_of_carts' => 'required|array',
        ]);
        ...
        // Simulate order creation and retrieval of order details
        $order = [
            ...
            'receiver_details' => $request->receiver_details,
            'charge_payer' => $request->charge_payer,
            'dm_tips' => $request->dm_tips,
            'guest_id' => $request->guest_id,
            'contact_person_name' => $request->contact_person_name,
            'contact_person_number' => $request->contact_person_number,
            'list_of_carts' => $request->list_of_carts,
        ];
        ...
        return response()->json(['message' => 'Order placed successfully', 'order_details' => $order], 200);
    }
    ...
    public function parcel_instructions(Request $request)
    {
        ...
        $instructions = ParcelDeliveryInstruction::where('status', 1);
