...
class ConversationController extends Controller
{
    public function messages_store(Request $request)
    {
        ...
        $messages = Message::where(['conversation_id' => $conversation->id])->latest();
    ...
    public function conversations(Request $request,$user_id)
    {
        $limit = $request['limit']??10;
        $offset = $request['offset']??1;

        $sender = UserInfo::where('user_id', $user_id)->first();
        if(!$sender){
            $sender = new UserInfo();
            $sender->user_id = $request?->user()?->id;
            $sender->f_name = $request?->user()?->f_name;
        ...
        $data =  [
            ...
            'total_size' => count($conversations),
    ...
    public function messages(Request $request)
    {
        ...
        if(isset($conversation)){
            ...
            $messages = Message::where(['conversation_id' => $conversation->id])->latest()->get();
        }else{
            $messages =[];
            $order=0;
        }
        ...
        $data =  [
            'total_size' => $messages? count($messages):0,
    ...
    public function dm_conversations(Request $request)
    {
        ...
        $conversations = Conversation::with('sender','receiver','last_message')->where(['sender_id' => $sender->id])->orWhere(['receiver_id' => $sender->id])->orderBy('last_message_time', 'DESC')->get();
