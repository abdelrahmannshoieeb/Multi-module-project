...
"));
        ...
        if (isset($request->options)) {
            foreach (array_values($request->options) as $key => $option) {
                ...
                $temp_variation['name'] = $option['required'] ?? 'off';
        ...
        $item->variations_all = $food_variations;
        ...
        $item->attributes = $request->has('attribute_id') ? $request->attribute_id : [];
        $item->add_ons = $request->has('addon_ids') ? $request->addon_ids : [];
    ...
    public function update(Request $request, $id)
    {
        ...
        if (isset($request->options)) {
            foreach (array_values($request->options) as $key => $option) {
                ...
                $temp_variation['min'] = $option['min'] ?? 0;
                $temp_variation['max'] = $option['max'] ?? 0;
                if ($option['min'] ?? 0 > 0 &&  $option['min'] ?? 0 > $option['max'] ?? 0) {
                    $validator->getMessageBag()->add('name', translate('messages.minimum_value_can_not_be_greater_then_maximum_value'));
                    return response()->json(['errors' => Helpers::error_processor($validator)]);
                }
                if (!isset($option['values'])) {
                ...
                if ($option['max'] ?? 0 > count($option['values'])) {
                    $validator->getMessageBag()->add('name', translate('messages.please_add_more_options_or_change_the_max_value_for') . $option['name']);
                    return response()->json(['errors' => Helpers::error_processor($validator)]);
                }
        ...
        $item->variations = $request->has('attribute_id') ? $variations : [];
        $item->price = $request->price;
